<template>
    <div class="settingsBtn grid place-items-center w-7 h-7 rounded cursor-pointer" @contextmenu.prevent="handleRightClick">
...
export default {
  methods: {
    handleRightClick(event) {
      // Emit the custom event with mouse coordinates
      this.$emit('right-click', { x: event.clientX, y: event.clientY, index: this.index });
    },
  },
};
</script>